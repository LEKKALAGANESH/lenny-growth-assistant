...
4. **DOMPurify Defense-in-Depth**: before being handed to the iframe's `srcDoc`, artifact HTML passes through `DOMPurify.sanitize()` (`ArtifactViewer.js`). The sandbox attribute alone already makes script *execution* safe (an opaque, unique origin with no access to the parent document, cookies, or same-origin resources) — DOMPurify's job here is catching what the sandbox doesn't: a `<base href>` or `<meta http-equiv="refresh">` rewriting relative links or forcing navigation, and DOMPurify's mutation-XSS detection catching malformed markup that could smuggle content past the sandbox boundary. `<script>` tags and inline event-handler attributes are deliberately kept allowed (`ADD_TAGS`/`ADD_ATTR`), since interactive generated widgets are a core feature — `<base>`/`<meta>` are explicitly forbidden (`FORBID_TAGS`).
